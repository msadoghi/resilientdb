...
#include "executor/multi_contract/x_manager/data_storage.h"
...
class EController : public ConcurrencyController {
  public:
    EController(DataStorage * storage, int window_size);
    virtual ~EController();
    ...
    ConcurrencyController::ModifyMap * GetChangeList(int64_t commit_id);
...
void Clear(int64_t commit_id);
  ...
  private:
    ...
    void RemovePreRecord(const uint256_t& address, 
        int64_t commit_id);
