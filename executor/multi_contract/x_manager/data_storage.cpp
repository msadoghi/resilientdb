...
std::pair<uint256_t,int64_t> DataStorage::Load(const uint256_t& key, bool) const {
int idx = GetHashKey(key);
...
int64_t DataStorage::StoreWithVersion(const uint256_t& key, const uint256_t& value, int version, bool ) {
int idx = GetHashKey(key);
    std::unique_lock lock(mutex_[idx]);
