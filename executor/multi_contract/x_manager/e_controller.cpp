#include "executor/multi_contract/x_manager/e_controller.h"
...
// ========================================================
ConcurrencyController::ModifyMap * EController::GetChangeList(int64_t commit_id){
  // read lock
  return &(changes_list_[commit_id]);
}

bool EController::TryLock(const uint256_t& address, int64_t owner, LockType type){
  if(aborted_[owner]){
    return false;
  }
  return true;
...
void EController::StoreInternal(const int64_t commit_id, const uint256_t& address, const uint256_t& value, int version) {
...
uint256_t EController::LoadInternal(const int64_t commit_id, const uint256_t& address, int version) {
...
void EController::Store(const int64_t commit_id, const uint256_t& address, const uint256_t& value, int version) {
  StoreInternal(commit_id, address, value, version);
}

uint256_t EController::Load(const int64_t commit_id, const uint256_t& address, int version) {
  return LoadInternal(commit_id, address, version);
}

bool EController::Remove(const int64_t commit_id, const uint256_t& key, int version) {
...
void EController::AppendPreRecord(const uint256_t& address, 
    int64_t commit_id, Data& data) {
  {
    ...
    #ifdef CDebug
    LOG(ERROR)<<"append commit set:"<<commit_id<<" address:"<<address<<" size:"<<commit_set.size();
    #endif
   
     std::stack<std::unique_ptr<DataInfo>> tmp;
     while(!commit_set.empty()){
      auto& it = commit_set.back();
       if( !committed_[it->commit_id] && (it->commit_id > commit_id|| it->version==-1)){
         //LOG(ERROR)<<"get data front address:"<<address<<" commit id:"<<commit_id<<" front:"<<it->commit_id<<" committed:"<<committed_[it->commit_id];
         //LOG(ERROR)<<"push :"<<commit_set.back()->commit_id<<" version:"<<it->version;
    ...
    if(data.state == LOAD){
      if(commit_set.empty()){
        ...
        #ifdef CDebug
      LOG(ERROR)<<"LOAD from db:"<<" version:"<<data.version<<" commit id:"<<commit_id<<" address:"<<address;
        #endif
          //assert(ret.second<=1);
      }
      else {
       auto& it = commit_set.back();
        ...
        #ifdef CDebug
      LOG(ERROR)<<"LOAD from :"<<it->commit_id<<" version:"<< it->data.version<<" address:"<<address<<" commit id:"<<commit_id<<" data version:"<<it->version;
        #endif
      }
    }
    else {
      if(commit_set.empty()){
          ...
          #ifdef CDebug
          LOG(ERROR)<<"LOAD from db:"<<" address:"<<address<<" commit id:"<<commit_id;
          #endif
          //assert(ret.second<=1);
      }
      else {
       auto& it = commit_set.back();
        ...
        #ifdef CDebug
        LOG(ERROR)<<"LOAD from :"<<it->commit_id<<" version:"<<it->data.version<<" address:"<<address<<" commit id:"<<commit_id;
        #endif
      }
    }
  ...
  if(change_set.empty()){
    #ifdef CDebug
    LOG(ERROR)<<"append commit set:"<<commit_id<<" address:"<<address<<" size:"<<change_set.size()<<" commit id:"<<commit_id<<" type:"<<data.state;
    #endif
...
void EController::RemovePreRecord(const uint256_t& address, 
    int64_t commit_id) {
    ...
    //std::set<int> locked;
  {
    ...
    */
    ...
    for(const auto& it : change_set){
      ...
      /*
      if(last_commit){
        LOG(ERROR)<<"get last commit:"<<last_commit->commit_id<<" commit id:"<<commit_id<<" address:"<<address;
      }
      */
      if(commit_set.empty() || commit_set.front()->commit_id != commit_id){
        ...
        if(commit_set.empty()){
          ...
          LOG(ERROR)<<"not the first one:"<<commit_set.front()->commit_id<<" wait for:"<<commit_id<<" abort:"<<aborted_[commit_set.front()->commit_id]<<" address:"<<address;
        }
#endif
      }
      if(last_commit){
    ...
    for(const auto& it : change_set){
      ...
      auto& commit_set = pre_commit_list_[address];
      #ifdef CDebug
          LOG(ERROR)<<"commit:"<<commit_id<<" address:"<<address<<" data version:"<<it.second.back().version<<" data sze:"<<commit_set.size();
      #endif
        while(!commit_set.empty()){
          auto &it = commit_set.front();
          if(committed_[it->commit_id]){
        ...
        if(commit_set.empty()){
          ...
          //LOG(ERROR)<<" commit id:"<<commit_id<<" trigger:"<<commit_set.front()->commit_id<<" wait:"<<wait_[commit_set.front()->commit_id];
    ...
    for(int i = it.second.size()-1; i >=0 && !done;--i){
      ...
      switch(op.state){
        ...
        case STORE:
        {
          ...
          #ifdef CDebug
            LOG(ERROR)<<"commit overlap:"<<it.first<<" data:"<<op.version<<" commit id:"<<commit_id<<" now:"<<v;
          #endif
            //assert(op.version <v);
          }
          done = true;
...
const std::vector<int64_t> &EController::GetRedo(){
  return redo_;
}
const std::vector<int64_t> &EController::GetDone(){
  return done_;
}
