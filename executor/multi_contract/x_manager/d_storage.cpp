#include "executor/multi_contract/x_manager/d_storage.h"
...
void InternalReset(const uint256_t& key, const uint256_t& value, int64_t version,
  std::map<uint256_t, std::pair<uint256_t,int64_t> > * db, std::shared_mutex * mutex ) {
...
std::pair<uint256_t,int64_t> InternalLoad(const uint256_t& key, 
...
bool InternalExist(const uint256_t& key,
        const std::map<uint256_t, std::pair<uint256_t,int64_t> > * db, 
        std::shared_mutex * mutex) {
  std::shared_lock lock(*mutex);
...
int64_t InternalGetVersion(const uint256_t& key,
        const std::map<uint256_t, std::pair<uint256_t,int64_t> > * db, 
        std::shared_mutex * mutex) {
  std::shared_lock lock(*mutex);
  auto it = db->find(key);
...
int64_t D_Storage::StoreWithVersion(const uint256_t& key, const uint256_t& value, int version, bool is_local) {
//LOG(ERROR)<<"storage key:"<<key<<" value:"<<value<<" ver:"<<version<<" is local:"<<is_local;
...
int64_t D_Storage::Store(const uint256_t& key, const uint256_t& value, bool is_to_local_view) {
  //LOG(ERROR)<<"store key:"<<key<<" value:"<<value<<" local:"<<is_to_local_view;
  int idx = GetHashKey(key);
  if(is_to_local_view){
...
std::pair<uint256_t,int64_t> D_Storage::Load(const uint256_t& key, bool is_local_view) const {
...
int64_t D_Storage::GetVersion(const uint256_t& key, bool is_local) const{
  ...
  if(is_local){
    ...
    //LOG(ERROR)<<"get local ver:"<<(c_s_.find(key)->second.second)<<" key:"<<key;
    if(ret==0){
...
void D_Storage::Reset(const uint256_t& key, const uint256_t& value, int64_t version, bool is_local){
  int idx = GetHashKey(key);
  if(is_local){
