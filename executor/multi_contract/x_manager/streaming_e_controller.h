...
#include "executor/multi_contract/x_manager/concurrency_controller.h"
...
class StreamingEController : public ConcurrencyController {
  public:
    ...
    typedef std::map<uint256_t, std::deque<std::unique_ptr<DataInfo>> > PreCommitList;
    //typedef std::map<uint256_t, std::map<int64_t, std::unique_ptr<DataInfo>> > PreCommitList;
...
std::vector<int64_t> FetchAbort();
  ...
  private:
    ...
    D_Storage * storage_;
    std::mutex mutex_[4096], change_list_mutex_[4096], valid_mutex_[4096], rb_mutex_, g_mutex_, abort_mutex_;
    //mutable std::shared_mutex mutex_[4096], change_list_mutex_[4096], valid_mutex_[4096], rb_mutex_;
