#include "executor/multi_contract/manager/d_storage.h"
...
//LOG(ERROR)<<"get version key:"<<key<<" islocal:"<<is_local;
  ...
  if(is_local){
    ...
    if(ret==0){
      ret = InternalGetVersion(key, &g_s_[idx], &g_mutex_[idx]);
...
void D_Storage::Reset(const uint256_t& key, const uint256_t& value, int64_t version, bool is_local){
  //LOG(ERROR)<<"set key:"<<key<<" value:"<<value<<" local:"<<is_local<<" version:"<<version;
  int idx = GetHashKey(key);
  if(is_local){
