#include "executor/multi_contract/manager/local_view.h"
...
void LocalView::store(const uint256_t& key, const uint256_t& value) {
  ...
  if (it == local_changes_.end()){
    std::pair<uint256_t,int64_t> load_value = controller_->GetStorage()->Load(key, /*is_from_local=*/true);
    local_changes_[key].push_back(Data(STORE, value, load_value.second, load_value.first));
  }
  else {
...
uint256_t LocalView::load(const uint256_t& key) {
  auto it = local_changes_.find(key);
  if (it == local_changes_.end()){
    std::pair<uint256_t,int64_t> value = controller_->GetStorage()->Load(key, /*is_from_local=*/true);
    local_changes_[key].push_back(Data(LOAD, value.first, value.second));
    return value.first;
  }
  //LOG(ERROR)<<"load key:"<<key<<" version:"<<it->second.back().version;
...
bool LocalView::remove(const uint256_t& key) {
  ...
  if (it == local_changes_.end()){
    std::pair<uint256_t,int64_t> load_value = controller_->GetStorage()->Load(key, /*is_from_local=*/true);
    local_changes_[key].push_back(Data(REMOVE, 0, load_value.second, load_value.first));
  }
  else {
    const Data& data = local_changes_[key].back();
    if(data.state == LOAD){
